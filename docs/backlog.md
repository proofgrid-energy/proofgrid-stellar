...
## Submission-ready first issue

**Deterministic gateway disagreement handling.** Define the lookup provider result shape and freshness assumptions, then add offline fixtures for unavailable, stale, matching and conflicting responses. Any disagreement or insufficient freshness must return `unknown`, never `verified`. No live transaction or customer data is required; existing verified, revoked and superseded behavior must remain unchanged.
